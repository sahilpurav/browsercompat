...
class GuardExpression {
  ...
  public getSymbolResolveTarget(): ts.Expression | null {
    ...
    if (
      ts.isBinaryExpression(this.operand) &&
      this.operand.operatorToken.kind === ts.SyntaxKind.InKeyword
    ) {
...
class GuardExprChain {
  ...
  public constructor(
    expressions: readonly GuardExpression[],
...
class GuardExprChecker {
  ...
  public evaluate(
    expr: ts.Expression,
    mustBePosOrNeg: boolean | null = null
  ): GuardExpression {
  ...
  private _evaluateChainOperands(
    expr: ts.Expression,
    op: ts.BinaryOperator,
    guardExprs: GuardExpression[]
  ): void {
    ...
    if (ts.isBinaryExpression(expr) && expr.operatorToken.kind === op) {
      ...
      guardExprs.push(
        this.evaluate(expr, op === ts.SyntaxKind.AmpersandAmpersandToken)
      );
    }
  }
  ...
  private _evaluateBinaryExpression() {
    ...
    if (isExprWithEquals || isExprWithNotEquals) {
      this._evaluateEqualityExpression(
        left,
        right,
        isExprWithNotEquals,
        isStrictEquality
  ...
  private _evaluateEqualityExpression(
    ...
    if (
      _isUndefinedExpression(right) ||
      (!isStrict && right.kind === ts.SyntaxKind.NullKeyword)
    ) {
      ...
      this._evaluateTypeofCheckExpression(
        left.expression,
        right.text,
        isNotEquals
      );
  ...
  private _evaluateTypeofCheckExpression(
    operand: ts.Expression,
    type: string,
    isNotEquals: boolean
  ): void {
  ...
  private _tryResolveSymbol(): ts.Symbol | null {
    ...
    if (this.m_propertyName !== null) {
      const targetType: ts.Type = this.m_typeChecker.getApparentType(
        this.m_typeChecker.getTypeAtLocation(this.m_propertyObj!)
      );

      resolvedSymbol = this.m_typeChecker.getPropertyOfType(
        targetType,
        this.m_propertyName
      );
 ...
 */
function _unwrapNegations(
  expr: ts.Expression
): readonly [ts.Expression, boolean] {
