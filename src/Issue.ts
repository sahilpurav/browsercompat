...
class Issue {
  ...
  public getMessage(): string {
    ...
    parts.push(
      ...
      this.featureName,
      ' with ',
      this.clientInfo.description,
      ':\n'
    );

    const hasStartVersion: boolean =
      Version.compare(this.startVersion, Version.minVal) !== 0;
    const hasEndVersion: boolean =
      Version.compare(this.endVersion, Version.infinite) !== 0;
...
function _appendMessageForIssueKind(
  kind: IssueKind,
  altOrPrefix: string | null,
  msgParts: string[]
): void {
  switch (kind) {
    ...
    case IssueKind.NEEDS_FLAG:
      msgParts.push(
        'This feature may require a configuration setting to be changed to work properly.'
      );
      break;
    case IssueKind.NOTE:
...
function _appendMessageForIssueApplicableVersions(
  startVersion: Version | null,
  endVersion: Version | null,
  msgParts: string[]
): void {
