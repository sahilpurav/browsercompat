...
export class Rule extends Rules.TypedRule {
  public static metadata: IRuleMetadata = {
    ...
    description:
      'Checks code for possible compatibility issues with a given list of browsers.',
...
function _createOrGetCachedCompatChecker(options: any): ClientCompatChecker {
  ...
  const showNotes: boolean = Boolean(
    'showNotes' in options ? options.showNotes : false
  );
  const showPartial: boolean = Boolean(
    'showPartialImplementations' in options
      ? options.showPartialImplementations
      : false
  );
  ...
  const checker = new ClientCompatChecker(
    _compatData,
...
function _canUseCachedCompatChecker(
  targets: any,
...
export function parseTargets(targetsArg: any): ClientInfo[] {
  ...
  for (const name in targetsArg) {
    ...
    clientInfos.push(
      new ClientInfo(name, _compatData.getDisplayName(name), start, end)
    );
  }
  return clientInfos;
}

function _parseTargetVersion(
  targetName: string,
  targetVersion: any
): [Version, Version] {
...
function _parseTargetVersionString(
  targetName: string,
