...
import GuardExprChecker, {
  GuardExprChain,
  GuardExpression
} from './GuardExprChecker';
...
class Walker extends AbstractWalker {
  ...
  private _visit(node: ts.Node): void {
    switch (node.kind) {
      ...
      case ts.SyntaxKind.ConditionalExpression:
        return this._visitConditionalExpression(
      ...
      case ts.SyntaxKind.IndexSignature:
        return this._visitFunctionDecl(
          node as ts.FunctionLikeDeclaration | ts.IndexSignatureDeclaration
        );
  ...
  private _reportIssues(node: ts.Node, issues: Issue[]) {
    ...
    for (let i: number = 0; i < issues.length; i++) {
      this.m_issuesFound.push(
        new IssueWithLocation(fileName, lc.line, lc.character, issues[i])
      );
  ...
  private _visitPropertyAccess(node: ts.PropertyAccessExpression): void {
    ...
    const targetType: ts.Type = this.m_typeChecker.getApparentType(
      this.m_typeChecker.getTypeAtLocation(node.expression)
    );
    ...
    const prop: ts.Symbol | undefined = this.m_typeChecker.getPropertyOfType(
      targetType,
      propName
    );
    if (prop === undefined) {
  ...
  private _visitElementAccess(node: ts.ElementAccessExpression): void {
    if (ts.isIdentifier(node.expression)) {
      const symbol:
        | ts.Symbol
        | undefined = this.m_typeChecker.getSymbolAtLocation(node.expression);
  ...
  private _checkAddEventListenerCall(node: ts.CallExpression): void {
    ...
    const targetType: ts.Type = this.m_typeChecker.getTypeAtLocation(
   ...
   */
  private _visitFunctionDecl(
    node: ts.FunctionLikeDeclaration | ts.IndexSignatureDeclaration
  ): void {
  ...
  private _visitVariableDecl(node: ts.VariableLikeDeclaration): void {
    ...
    const initializer: ts.Expression | null = _getInitializerForDeclaration(
      node
    );
    if (initializer !== null) {
  ...
  private _visitIfStatement(node: ts.IfStatement): void {
    return this._visitIfStatementOrExpr(
      node.expression,
      node.thenStatement,
      node.elseStatement
    );
  ...
  private _visitIfStatementOrExpr(
    ...
    if (guardExprOrChain instanceof GuardExpression) {
      ...
      const isOr: boolean =
        guardExprOrChain.operator === ts.SyntaxKind.BarBarToken;
   ...
   */
  private _visitGuardExpressionAndPushSymbol(
    guardExpr: GuardExpression,
   ...
   */
  private _visitGuardExprChainAndPushSymbols(
    chain: GuardExprChain,
    sourceExpr: ts.Expression
  ): void {
  ...
  private _getNodeApparentType(node: ts.Node): ts.Type {
    return this.m_typeChecker.getApparentType(
      this.m_typeChecker.getTypeAtLocation(node)
    );
  }
  ...
  private _getDeclaringType(symbol: ts.Symbol): ts.Type | null {
    ...
    for (let i: number = 0; i < symbol.declarations.length; i++) {
      ...
      const nameNode = (declNode as
        | ts.ClassDeclaration
        | ts.InterfaceDeclaration).name;
  ...
  private _isTypeRecognizedByCompatChecker(type: ts.Type): boolean {
    return (
      type.symbol && this.m_compatChecker.isGlobalRecognized(type.symbol.name)
    );
  }
  ...
  private _checkType(
    ...
    if ((type.flags & ts.TypeFlags.UnionOrIntersection) !== 0) {
      return this._checkUnionOrIntersectionType(
        type as ts.UnionOrIntersectionType,
        node,
        recurse
      );
    }
   ...
   */
  private _checkGenericTypeArguments(
    type: ts.Type,
    node: ts.Node,
    recurse: boolean = true
  ): void {
  ...
  private _checkPropertyOrEvent(
    node: ts.Node,
    type: ts.Type,
    prop: ts.Symbol | string,
    isStatic: boolean,
    isEvent: boolean,
    checkBaseTypes: boolean
  ): void {
  ...
  private _getPropertyOrEventIssues(
    ...
    this.m_compatChecker.checkPropertyOrEvent(
      typeName,
      propName,
      isEvent,
      issues
...
const _staticTypeNames: WeakMap<ts.Type, string> = new WeakMap<
  ts.Type,
  string
>();
...
function _isPropertyDefinedInLibrary(
  propSymbol: ts.Symbol | null,
  declaringType: ts.Type | null = null
): boolean {
  if (propSymbol !== null) {
 ...
 */
function _getCompatCheckerTypeName(
  type: ts.Type,
  isStatic: boolean
): string | null {
  if (!type.symbol) {
  ...
  const rawName: string = isStatic
    ? _getCompatCheckerStaticTypeName(type)
    : type.symbol.name;
...
function _isTypeOfTS36WindowGlobal(type: ts.Type): boolean {
  ...
  return (
    (name1 === _WINDOW_TYPE && name2 === _GLOBAL_THIS_TYPE) ||
 ...
 */
function _getInitializerForDeclaration(
...
function _canIssuesBeSuppressedByGuards(issues: Issue[]): boolean {
  for (let i: number = 0; i < issues.length; i++) {
    ...
    if (
      issue.kind === IssueKind.NOT_SUPPORTED ||
